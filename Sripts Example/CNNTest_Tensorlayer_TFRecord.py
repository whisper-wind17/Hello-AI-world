...
with tf.device('/cpu:0'):
    ...
    for epoch in range(NUM_EPOCH):
        ...
        line = "TimeCost: %.1f sec, epoch: %d/%d, train_loss: %.8f, train_acc: %.8f\n" % (time.time()-start_time, epoch, NUM_EPOCH, train_loss, train_acc)
        print(line)
        ...
        print("   test loss: %.8f" % (test_loss/ n_batch))
        print("   test acc: %.8f" % (test_acc/ n_batch))
